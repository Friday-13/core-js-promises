...
function queuPromises(promises) {
  function reducePromiseChain(chainHead, currentPromise) {
    return chainHead.then((headValue) => {
      return currentPromise.then((promiseValue) => headValue + promiseValue);
    });
  }
  return promises.reduce(reducePromiseChain, Promise.resolve(''));
}
